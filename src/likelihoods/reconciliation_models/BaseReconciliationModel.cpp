...
void BaseReconciliationModel::fillNodesPostOrder(corax_rnode_t *node,
...
void BaseReconciliationModel::fillPrunedNodesPostOrder(corax_rnode_t *node,
    std::vector<corax_rnode_t *> &nodes)
{
...
void BaseReconciliationModel::setFractionMissingGenes(const std::string &fractionMissingFile)
{
  ...
  for (unsigned int i = 0; i < _speciesTree.getLeafNumber(); ++i) {
    if (_fm[i] == -1.0) {
      Logger::error << "Error: the fraction missing file " << fractionMissingFile
                    << " does not cover species " << _speciesTree.getNode(i)->label
                    << std::endl;
      assert(false);
    }
  }
