    ...
    ParallelOfstream &os)
{
  ...
  os << ":H=" << ((event.type == ReconciliationEventType::EVENT_T ||
                   event.type == ReconciliationEventType::EVENT_TL) ? "Y" : "N");
    ...
    ParallelOfstream &os)
{
  if(node->next) {
    ...
    os << "(";
    recursivelySaveReconciliationsNHX(speciesTree, left, depth + 1, geneToEvents, os);
    os << ",";
    recursivelySaveReconciliationsNHX(speciesTree, right, depth + 1, geneToEvents, os);
    os << ")";
  }
  if (node->label) {
    os << node->label;
    ...
    ParallelOfstream &os)
{
  ...
  recursivelySaveReconciliationsNHX(speciesTree, &virtualRoot, 0, geneToEvents, os);
  os << ";";
  os << std::endl;
}
    ...
    ParallelOfstream &os)
{
  if(node->next) {
    ...
    os << "(";
    recursivelySaveReconciliationsALE(speciesTree, left, depth + 1, geneToEvents, os);
    os << ",";
    recursivelySaveReconciliationsALE(speciesTree, right, depth + 1, geneToEvents, os);
    os << ")";
  }
    ...
    ParallelOfstream &os)
{
  ...
  virtualRoot.label = nullptr;
  virtualRoot.length = 0.0;
  recursivelySaveReconciliationsALE(speciesTree, &virtualRoot, 0, geneToEvents, os);
  os << ";";
  os << std::endl;
}
    ...
    ParallelOfstream &os)
{
  ...
  if (event.type != ReconciliationEventType::EVENT_L) {
    if ((previousEvent->type == ReconciliationEventType::EVENT_T && geneIndex == previousEvent->rightGeneIndex) ||
        previousEvent->type == ReconciliationEventType::EVENT_TL)
    {
      corax_rnode_t *previousEventSpeciesDest = speciesTree->nodes[previousEvent->destSpeciesNode];
      assert(previousEventSpeciesDest->label);
      os << indent << "\t<transferBack destinationSpecies=\"" << previousEventSpeciesDest->label << "\"/>" << std::endl;
    }
  }
  switch(event.type) {
  case ReconciliationEventType::EVENT_None:
  ...
  case ReconciliationEventType::EVENT_T:
  case ReconciliationEventType::EVENT_TL:
    os << indent << "\t<branchingOut speciesLocation=\"" << species->label << "\"/>" << std::endl;
    break;
  case ReconciliationEventType::EVENT_L:
    os << indent << "\t<loss speciesLocation=\"" << species->label << "\"/>" << std::endl;
    break;
  default:
    break;
  }
...
static void recursivelySaveGeneTreeRecPhyloXML(corax_rtree_t *speciesTree,
    unsigned int geneIndex,
    std::vector<std::vector<Scenario::Event> > &geneToEvents,
    const Scenario::Event *previousEvent,
    std::string &indent,
    ParallelOfstream &os)
{
  ...
  for (unsigned int i = 0; i < events.size() - 1; ++i) {
    ...
    writeEventRecPhyloXML(speciesTree, geneIndex, event, previousEvent, indent, os);
    previousEvent = &event;
    ...
    if (event.type == ReconciliationEventType::EVENT_SL) {
      ...
      lossEvent.speciesNode = lostSpecies->node_index;
    } else {
      lossEvent.speciesNode = event.speciesNode;
    }
    os << indent << "<clade>" << std::endl;
    indent += "\t";
    os << indent << "<name>loss</name>" << std::endl;
    writeEventRecPhyloXML(speciesTree, geneIndex, lossEvent, previousEvent, indent, os);
    indent.pop_back();
    os << indent << "</clade>" << std::endl;
  }
  os << indent << "<clade>" << std::endl;
  ...
  writeEventRecPhyloXML(speciesTree, geneIndex, event, previousEvent, indent, os);
  if (!event.isLeaf()) {
    recursivelySaveGeneTreeRecPhyloXML(speciesTree, event.leftGeneIndex, geneToEvents, &event, indent, os);
    recursivelySaveGeneTreeRecPhyloXML(speciesTree, event.rightGeneIndex, geneToEvents, &event, indent, os);
  }
  indent.pop_back();
  os << indent << "</clade>" << std::endl;
  for (unsigned int i = 0; i < events.size() - 1; ++i) {
...
static void saveGeneTreeRecPhyloXML(corax_rtree_t *speciesTree,
    unsigned int virtualRootIndex,
    std::vector<std::vector<Scenario::Event> > &geneToEvents,
    ParallelOfstream &os)
{
  ...
  recursivelySaveGeneTreeRecPhyloXML(speciesTree, virtualRootIndex, geneToEvents, &previousEvent, indent, os);
  os << "</phylogeny>" << std::endl;
  os << "</recGeneTree>" << std::endl;
}
    ...
    ParallelOfstream &os)
{
  if(node->next) {
    ...
    os << "(";
    recursivelySaveReconciliationsNewickEvents(left, depth + 1, geneToEvents, os);
    os << ",";
    recursivelySaveReconciliationsNewickEvents(right, depth + 1, geneToEvents, os);
    os << ")";
  }
  if (!node->next) {
    os << (node->label ? node->label : "null");
      ...
      ParallelOfstream &os)
{
  ...
  virtualRoot.label = nullptr;
  virtualRoot.length = 0.0;
  recursivelySaveReconciliationsNewickEvents(&virtualRoot, 0, geneToEvent, os);
  os << ";";
  os << std::endl;
}
