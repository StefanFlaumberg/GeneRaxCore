...
public:
   ...
   *  a newick string
   */
  static std::unique_ptr<PLLUnrootedTree>
   ...
   */
  static std::unique_ptr<PLLUnrootedTree>
  buildWithOutgroupFromRooted(const PLLRootedTree &rootedTree,
                              const std::string &outgroup);

  static std::unique_ptr<PLLUnrootedTree>
  buildWithOutgroupFromRootedNewick(const std::string &newickRootedStr,
                                    const std::string &outgroup);
   ...
   */
  static std::string
  buildConsensusTree(const std::vector<std::shared_ptr<PLLUnrootedTree>> &trees,
                     double threshold = 0.51);
  ...
  std::string getNewickString(UnodePrinter f = defaultUnodePrinter,
                              ...
                              bool rooted = false);
  static std::string
  getRootedNewickString(corax_unode_t *root,
                        UnodePrinter f = defaultUnodePrinter);
  static std::string getSubtreeString(corax_unode_t *subtree,
                                      UnodePrinter f = defaultUnodePrinter);
  ...
  static corax_unode_t *getRight(corax_unode_t *node) {
