...
PLLUnrootedTree::buildFromStrOrFile(const std::string &strOrFile) {
...
std::unique_ptr<PLLUnrootedTree>
PLLUnrootedTree::buildWithOutgroupFromRootedNewick(
    const std::string &newickRootedStr, const std::string &outgroup) {
...
std::unique_ptr<PLLUnrootedTree>
PLLUnrootedTree::buildWithOutgroupFromRooted(const PLLRootedTree &rootedTree,
...
void PLLUnrootedTree::setLabel(unsigned int nodeIndex,
                               const std::string &label) {
  setNodeLabel(getNode(nodeIndex), label);
}

std::string
PLLUnrootedTree::buildConsensusTree(std::vector<std::string> &strOrFiles,
                                    double threshold) {
...
unsigned int PLLUnrootedTree::getLeafNumber() const { return _tree->tip_count; }
...
struct ToSort {
  ...
  ToSort() {}
  ToSort(corax_unode_t *inode, const std::string &ilabel,
         unsigned int idistanceToLeaf)
      : node(inode), label(ilabel), distanceToLeaf(idistanceToLeaf) {}
  ToSort(const ToSort &s)
      : node(s.node), label(s.label), distanceToLeaf(s.distanceToLeaf) {}
};

static void fillBranchesRec(corax_unode_t *node,
                            std::vector<ToSort> &toSortVector, ToSort &toSort) {
  if (!node->next) {
...
static void computePairwiseDistancesRec(corax_unode_t *currentNode,
  ...
  computePairwiseDistancesRec(currentNode->next->back, currentDistance,
                              distancesVector);
  computePairwiseDistancesRec(currentNode->next->next->back, currentDistance,
                              distancesVector);
}
...
void PLLUnrootedTree::computePairwiseDistances(MatrixDouble &distances,
  ...
  for (unsigned int i = 0; i < MIter; ++i) {
    ...
    if (node->next) {
      ...
      computePairwiseDistancesRec(node->next->back, 0.0, distancesVector);
      computePairwiseDistancesRec(node->next->next->back, 0.0, distancesVector);
...
static bool orientAux(corax_unode_t *u, corax_unode_t **pv,
...
void PLLUnrootedTree::orientTowardEachOther(
    corax_unode_t **pu, corax_unode_t **pv,
...
static void printAux(corax_unode_t *node, std::stringstream &ss,
                     UnodePrinter f) {
  if (node->next) {
    ss << "(";
    auto temp = node->next;
...
std::string PLLUnrootedTree::getSubtreeString(corax_unode_t *subtree,
                                              UnodePrinter f) {
  if (!subtree) {
    return "null";
  }
...
std::string PLLUnrootedTree::getNewickString(UnodePrinter f,
                                             corax_unode_t *root, bool rooted) {
  if (!root) {
...
searchRoot(corax_unode_t *node, unsigned int &visited,
  ...
  if (!node->next) {
    ...
    unsigned int visited1 = 0;
    auto res1 = searchRoot(left, visited1, targetLeaves);
    visited += visited1;
    if (res1) {
      return res1;
    }
    unsigned int visited2 = 0;
    auto res2 = searchRoot(right, visited2, targetLeaves);
    visited += visited2;
    if (res2) {
      return res2;
    }
...
void PLLUnrootedTree::ensureUniqueLabels() {
  ...
  for (auto node : getPostOrderNodes()) {
    if (node->next) {
      ...
      free(node->label);
      node->label =
          static_cast<char *>(malloc(sizeof(char) * (newLabel.size() + 1)));
